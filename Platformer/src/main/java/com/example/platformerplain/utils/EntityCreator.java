...
//    }
    ...
    public static Entity createEntity(String entityType, int x, int y, int w, int h, int tag) {
        return switch (entityType) {
            ...
            // destination
            case IGLOO_, OASIS_ -> new DestinationBlock(x, y, w, h, Main.gameState.map.index, loadImageView(entityType, tag));
            // enemy blocks
            case FIRE_DRAGON_ -> new FireDragonBlock(x, y, w, h, loadImageView(entityType, tag), Main.rangeIterator.next());
            case MUMMY_ -> new MummyBlock(x, y, w, h, loadImageView(entityType, tag), Main.rangeIterator.next());
            // moving object blocks
            case FLYING_CARPET_ -> new FlyingCarpetBlock(x, y, w, h, loadImageView(entityType, tag), Main.rangeIterator.next());
